-- Stelposten (handmatige kostenpost zonder offerte), sejourskosten per crewlid, en een
-- CO2-indicatie op crew/leveranciers. Voer dit één keer uit in de Supabase SQL Editor.

-- Handmatige kostenpost: gebruikt als er (nog) geen gekozen offerte is voor een categorie.
-- Werkt voor elke categorie (bv. "Kilometervergoeding"), niet alleen een vaste lijst.
...
-- Sejours (dagvergoeding) per crewlid, gebruikt zolang ze "Hotel nodig" hebben.
alter table public.crew_members add column if not exists per_diem_rate numeric not null default 0;

-- Optionele CO2-inschatting per offerte (bv. transport/logistiek van een leverancier).
alter table public.quotes add column if not exists co2_kg numeric;

-- get_shared_project: kostprijs valt terug op manual_cost als er geen gekozen offerte is,
-- zodat stelposten ook in de klant-begroting meetellen.
