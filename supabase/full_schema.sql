...
-- ========== LOCATIECATALOGUS (VENUES) ==========
-- Een herbruikbare bibliotheek van locaties met hun technische specificaties (stroom,
-- laad-in-toegang, afmetingen, rigging, wifi, contactpersoon), zodat producers deze info
-- niet steeds opnieuw hoeven te verzamelen als ze een eerder gebruikte locatie weer boeken
-- voor een nieuw event. Zelfde eigenaarschapsmodel als leveranciers (suppliers): user_id =
-- eigenaar, teamleden mogen lezen/bewerken, alleen team-admins mogen verwijderen (identiek
-- aan de policies op public.suppliers).
...
-- ========================================================================
-- ACTUAL COSTS + LICHTE FACTURATIE (appended by agent-a55fdcb403fafee44)
-- Zie supabase/migrations_actual_costs.sql voor deze migratie als losse file.
-- ========================================================================
...
-- ========== SPREKERS (speaker management, showcaller/crew portal) ==========
...
-- bewust dezelfde save_attendee_contact-RPC hierboven, dus dezelfde privacyregels (alleen
-- mogelijk als het doelwit zelf networking_opt_in=true heeft) als handmatig bewaren via de
-- Netwerk-tab. Geen nieuwe RPC nodig voor het opslaan; alleen de kolom hieronder plus een
-- (server-side, admin-client) lookup op qr_token in de nieuwe route/pagina.
...
-- Self-adaptieve to-do lijst per project: organisatiebrede standaardsjabloon
-- (todo_templates), eenmalig geseed en daarna vrij aanpasbaar, gekopieerd naar
-- losse project_todos bij eerste bezoek van een project.
...
-- Bijlagen per rider-onderdeel — zelfde patroon als intake_checklist_photos.
...
-- losgekoppeld van rental_period_multipliers (externe verhuurcatalogus).
-- Eigen, per-organisatie bewerkbare huurperiode-staffel voor eigen materiaal (Materiaalbeheer),
...
-- Postgres treats create_project_secure met een langere parameterlijst als een nieuwe
-- overload i.p.v. een vervanging — de oude 14-parameter versie expliciet opruimen.
...
-- === migrations_catering_allergies.sql ===
