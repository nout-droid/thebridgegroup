...
export async function generateSchedulePdf(data: SchedulePdfData, branding: OrgBranding): Promise<Buffer> {
  registerFonts();
  const logoBuffer = await resolveLogoBuffer(branding);
  const generatedAt = data.generatedAt.toLocaleString("nl-NL");

  return renderToBuffer(<Document>{buildSchedulePage(data, logoBuffer, generatedAt, branding)}</Document>);
}
