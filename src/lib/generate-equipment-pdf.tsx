...
export function buildEquipmentPage(
...
export async function generateEquipmentPdf(data: EquipmentPdfData, branding: OrgBranding): Promise<Buffer> {
  registerFonts();
  const logoBuffer = await resolveLogoBuffer(branding);
  const generatedAt = data.generatedAt.toLocaleString("nl-NL");

  return renderToBuffer(<Document>{buildEquipmentPage(data, logoBuffer, generatedAt, branding)}</Document>);
}
