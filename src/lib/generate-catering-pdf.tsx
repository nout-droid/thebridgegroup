...
export function buildCateringPage(
  data: CateringPdfData,
  logoBuffer: Buffer,
  generatedAt: string,
...
export async function generateCateringPdf(data: CateringPdfData, branding: OrgBranding): Promise<Buffer> {
  registerFonts();
  const logoBuffer = await resolveLogoBuffer(branding);
  const generatedAt = data.generatedAt.toLocaleString("nl-NL");

  return renderToBuffer(<Document>{buildCateringPage(data, logoBuffer, generatedAt, branding)}</Document>);
}
