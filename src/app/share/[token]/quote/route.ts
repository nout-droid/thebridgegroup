...
// Zelfde opzet als projects/[id]/budget/quote/route.ts, maar token-scoped zodat de klant deze
// zelf kan downloaden vanuit het portaal — geen sessie nodig, alleen het share_token.
export async function GET(
  ...
  const { data: project } = await admin
    .from("projects")
    .select(
      "id, name, client_name, event_date, user_id, quote_number, quote_date, quote_notes, client_reference, share_token, signature_url, signature_signed_by, signature_signed_at"
    )
