...
import { AANVRAGEN_SUB_NAV_LABELS, FLIGHT_SECTION_LABELS } from "../labels";

export default async function SupplierFlightRequestsPage({
  params,
  searchParams,
}: {
  ...
  return (
    <SupplierTranslatorProvider
      staticLabels={[...SUPPLIER_NAV_LABELS, ...AANVRAGEN_SUB_NAV_LABELS, ...FLIGHT_SECTION_LABELS]}
    ...
    >
      <div className="flex min-h-screen flex-col">
        ...
        <main className="mx-auto w-full max-w-4xl flex-1 space-y-6 px-6 py-8">
          ...
          />
          {selectedProject && (
            <SupplierFlightSection supplierId={supplierId} projectId={selectedProject.id} members={crewMembers} />
          )}
        </main>
        <Footer />
      </div>
